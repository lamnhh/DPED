...
with tf.Session(config=config) as sess:
    ...
    if phone.endswith("_orig"):
        ...
        for photo in tqdm(test_photos):
            ...
            image = np.float16(misc.imresize(misc.imread(test_dir + photo, mode="RGB"), res_sizes[phone])) / 255
            ...
            enhanced_2d = np.array(sess.run(enhanced, feed_dict={x_: image_crop_2d})[0]) * 255
            enhanced_2d = np.clip(enhanced_2d, 0, 255)
            enhanced_2d = enhanced_2d.astype("uint8")

            original_image = Image.open(test_dir + photo).resize((256, 256))
            enhanced_image = Image.fromarray(enhanced_2d).resize((256, 256))
            ...
            plt.imsave("results/" + photo, np.array(enhanced_image))
